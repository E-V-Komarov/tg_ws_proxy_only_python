...
# -*- coding: utf-8 -*-
"""
Telegram WebSocket Bridge Proxy – консольная версия.
Запускает SOCKS5-сервер, который перенаправляет трафик Telegram через WebSocket.
"""
...
# Сопоставление IP → (DC ID, is_media)
...
# Структуры для упаковки/распаковки
...
async def _bridge_ws(reader, writer, ws: RawWebSocket, label,
    ...
    try:
        ...
        log.info("[%s] %s (%s) WS session closed: ^%s (%d pkts) v%s (%d pkts) in %.1fs",
...
async def _bridge_tcp(reader, writer, remote_reader, remote_writer, label,
                      dc=None, dst=None, port=None, is_media=False):
...
async def _handle_client(reader, writer):
    ...
    try:
        ...
        if ':' in dst:
            log.error("[%s] IPv6 address detected: %s:%d — IPv6 not supported", label, dst, port)
            writer.write(_socks5_reply(0x05))
            await writer.drain()
            writer.close()
            return

        # Не-Telegram трафик – прямой проброс
        ...
        # Telegram – принимаем SOCKS, читаем init-пакет
        writer.write(_socks5_reply(0x00))
        await writer.drain()
        ...
        if _is_http_transport(init):
            ...
            log.debug("[%s] HTTP transport to %s:%d (rejected)", label, dst, port)
            writer.close()
            return
        ...
        if dc is None and dst in _IP_TO_DC:
            dc, is_media = _IP_TO_DC[dst]
            if dc in _dc_opt:
                init = _patch_init_dc(init, -dc if is_media else dc)
                init_patched = True

        if dc is None or dc not in _dc_opt:
            log.warning("[%s] unknown DC%s for %s:%d -> TCP passthrough", label, dc, dst, port)
        ...
        if dc_key in _ws_blacklist:
            log.debug("[%s] DC%d%s WS blacklisted -> TCP %s:%d", label, dc, media_tag, dst, port)
        ...
        if ws:
            ...
            for domain in domains:
                log.info("[%s] DC%d%s -> %s via %s", label, dc, media_tag, domain, target)
                try:
        ...
        if ws is None:
            ...
            log.info("[%s] DC%d%s -> TCP fallback to %s:%d", label, dc, media_tag, dst, port)
            ok = await _tcp_fallback(reader, writer, dst, port, init, label, dc=dc, is_media=is_media)
            if ok:
                log.info("[%s] DC%d%s TCP fallback closed", label, dc, media_tag)
            return
        ...
        if proto is not None and (init_patched or is_media or proto != _PROTO_INTERMEDIATE):
            try:
                ...
                pass
        await ws.send(init)
        await _bridge_ws(reader, writer, ws, label, dc=dc, dst=dst, port=port, is_media=is_media, splitter=splitter)
...
# ------------------------- Запуск прокси -------------------------
async def _run(port: int, dc_opt: Dict[int, Optional[str]], stop_event: Optional[asyncio.Event] = None, host: str = '127.0.0.1'):
    global _dc_opt
    _dc_opt = dc_opt
    ...
    log.info("=" * 60)
    log.info("  Telegram WS Bridge Proxy (консольная версия)")
    log.info("  Listening on   %s:%d", host, port)
    log.info("  Target DC IPs:")
    for dc, ip in dc_opt.items():
        log.info("    DC%d: %s", dc, ip)
    log.info("=" * 60)
    log.info("  Настройте Telegram Desktop:")
    log.info("    SOCKS5 прокси -> %s:%d (без логина/пароля)", host, port)
    log.info("=" * 60)
    ...
    async def log_stats():
        try:
            while True:
                ...
                bl = ', '.join(f'DC{d}{"m" if m else ""}' for d, m in sorted(_ws_blacklist)) or 'none'
...
def main():
    import webbrowser
    ap = argparse.ArgumentParser(description='Telegram WebSocket Bridge Proxy (консоль)')
    ap.add_argument('--port', type=int, default=DEFAULT_PORT, help=f'Порт для прослушивания (default {DEFAULT_PORT})')
    ap.add_argument('--host', type=str, default='127.0.0.1', help='Адрес для прослушивания (default 127.0.0.1)')
    ap.add_argument('--dc-ip', metavar='DC:IP', action='append', default=[],
                    help='Целевой IP для DC, например --dc-ip 2:149.154.167.220')
    ap.add_argument('-v', '--verbose', action='store_true', help='Подробное логирование')
    args = ap.parse_args()
    ...
    listening_ip = args.host
    listening_port = args.port
    tg_link = f"tg://socks?server={listening_ip}&port={listening_port}"
    print("Откройте в Telegram:", tg_link)
